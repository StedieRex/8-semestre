...
def camara():
    ...
    while capture.isOpened():
        ...
        if ret:
            ...
            result = cv2.bitwise_and(mascara_delimitadora, mascara_delimitadora, mask=mascara)

            # Ajustar brillo y contraste en las áreas resaltadas
            result_resaltado = adjust_brightness_contrast(result, alpha=1.5, beta=50)

            # Mostrar la imagen enmascarada y el video original
            cv2.imshow('Original', frame)
            cv2.imshow('Resaltado Blanco y Amarillo', result_resaltado)
